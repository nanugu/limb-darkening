...
def overplot_analytical_laws(ctx, fit_bundle, band_view='H', savefig=0):
    ...
    display_names = {
        'Power Law': 'Power Law fit',
        'SATLAS': 'SATLAS fit',
        'Seed Power Law': 'Seed fit',
    }

    pl = fit_bundle['pl']
    sal = fit_bundle.get('sal')
    pl_seed = fit_bundle.get('pl_seed')
    chi2 = fit_bundle['chi2']
    ...
    if isinstance(pl_seed, dict):
        models['Seed Power Law'] = {
            'diam': _f(pl_seed.get('diam')),
            'H,alpha': _f(pl_seed.get('H,alpha')),
            'K,alpha': _f(pl_seed.get('K,alpha')),
            'H,diam': '$diam',
            'H,profile': '$MU**$H,alpha',
            'H,spectrum': '($WL<1.8)',
            'K,diam': '$diam',
    ...
    model_styles = {
        ...
        'Seed Power Law': {'color': '#7A3E9D', 'chi2': _f(chi2.get('pl_seed')), 'linestyle': '-.'},
    }
