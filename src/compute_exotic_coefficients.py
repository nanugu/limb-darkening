...
OUTPUT_FILE = "compute_exotic_coefficients_LONG.csv"
GRID_PUBLIC = {
    "kurucz": "Kurucz",
    "stagger": "Stagger",
    "mps1": "MPS1",
...
SATLAS_LOCAL_DIR = Path(__file__).resolve().parent
...
def _parse_tabulated_array(value: str | list | tuple | np.ndarray) -> np.ndarray:
...
def _satlas_law_fit(law: str, mu: np.ndarray, intensity: np.ndarray) -> tuple[np.ndarray, np.ndarray, str]:
    mu = np.asarray(mu, dtype=float).reshape(-1)
    intensity = np.asarray(intensity, dtype=float).reshape(-1)
    mask = np.isfinite(mu) & np.isfinite(intensity) & (mu >= 0.0) & (mu <= 1.0)
    mu = mu[mask]
    intensity = intensity[mask]
    if mu.size < 4:
        return np.full(2 if law in {"power2", "quadratic"} else 1, np.nan), np.full(2 if law in {"power2", "quadratic"} else 1, np.nan), 'insufficient_points'
    order = np.argsort(mu)
    mu = mu[order]
    ...
    if law == 'linear':
        ...
        func = power2_ld_law
        p0 = [0.6, 0.3]
        bounds = ([0.0, 0.0], [2.0, 2.5])
    elif law == 'quadratic':
        func = quadratic_ld_law
        p0 = [0.3, 0.3]
    ...
    try:
        popt, pcov = curve_fit(func, mu, intensity, p0=p0, bounds=bounds, maxfev=20000)
        perr = np.sqrt(np.diag(pcov)) if pcov.ndim == 2 else np.full(len(popt), np.nan)
        return np.asarray(popt, dtype=float), np.asarray(perr, dtype=float), ''
    except Exception as exc:
        npar = 2 if law in {"power2", "quadratic"} else 1
        return np.full(npar, np.nan), np.full(npar, np.nan), f'curve_fit_fail:{exc}'


def _get_local_satlas_model(teff: float, logg: float, mass: float | None = None) -> Path:
    local_files = {p.name: p for p in SATLAS_LOCAL_DIR.glob('ld_satlas_surface*.dat')}
    if not local_files:
        raise FileNotFoundError(f'No local SATLAS files found in {SATLAS_LOCAL_DIR}')

    candidates = []
    for idx, base in enumerate(satlas.rossFilesSph):
        if base in local_files:
            dist = ((satlas.rossTable['Teff'][idx] - float(teff)) / 100.0) ** 2 + (10.0 * (satlas.rossTable['logg'][idx] - float(logg))) ** 2
            if mass is not None and np.isfinite(mass):
                dist += (satlas.rossTable['mass'][idx] - float(mass)) ** 2
            candidates.append((dist, idx, local_files[base]))
    if not candidates:
        raise FileNotFoundError(f'No matching local SATLAS models available in {SATLAS_LOCAL_DIR}')
    candidates.sort(key=lambda item: item[0])
    return candidates[0][2]


def _compute_satlas_cliv(teff: float, logg: float, *, band: str, mass: float | None = None) -> tuple[np.ndarray, np.ndarray]:
    filename = _get_local_satlas_model(float(teff), float(logg), mass=mass)
    component = band.upper()
    model = satlas.readFile(str(filename), band=component, component=component, verbose=False)
    mu_native = _parse_tabulated_array(model[f'{component},_MUtab'])
    intens = _parse_tabulated_array(model[f'{component},_Itab'])
    if intens.size == 0:
        raise ValueError('No SATLAS intensity points available')
    norm = np.nanmax(intens)
    if np.isfinite(norm) and norm > 0:
        intens = intens / norm
    order = np.argsort(mu_native)
    mu_native = mu_native[order]
    intens = intens[order]
    mu_native, idx = np.unique(mu_native, return_index=True)
    intens = intens[idx]
    return mu_native, intens


def _satlas_mu_to_rosseland_mu(teff: float, logg: float, mass: float | None, mu_native: np.ndarray) -> np.ndarray:
    filename = satlas.getClosestModel(
        float(teff),
        float(logg),
        mass=None if mass is None or not np.isfinite(mass) else float(mass),
        modeltype="spheric",
    )
    base = Path(filename).name
    idx = satlas.rossFilesSph.index(base)
    ross_over_outer = float(satlas.rossTable["Ross/Outer"][idx])
    mu_native = np.asarray(mu_native, dtype=float).reshape(-1)
    p_over_outer = np.sqrt(np.clip(1.0 - mu_native**2, 0.0, 1.0))
    p_over_ross = p_over_outer / ross_over_outer
    mu_ross = np.sqrt(np.clip(1.0 - p_over_ross**2, 0.0, 1.0))
    mu_ross[p_over_ross > 1.0] = np.nan
    return mu_ross


def band_coeffs_satlas(
    *,
    teff: float,
    logg: float,
    band_label: str,
    ...
    mu_min: float = 0.0,
    use_rosseland_mu: bool = False,
) -> Dict[str, Dict[str, np.ndarray]]:
    mu, intensity = _compute_satlas_cliv(teff, logg, band=band_label, mass=mass)
    if use_rosseland_mu:
        mu = _satlas_mu_to_rosseland_mu(teff, logg, mass, mu)
    mask = np.isfinite(mu) & np.isfinite(intensity) & (mu >= float(mu_min))
    mu = mu[mask]
    intensity = intensity[mask]
    results: Dict[str, Dict[str, np.ndarray]] = {}
    for law in ('linear', 'quadratic', 'power1', 'power2'):
        coeffs, sigmas, note = _satlas_law_fit(law, mu, intensity)
        results[law] = {'coeffs': coeffs, 'sigmas': sigmas, 'note': note}
    return results


def run_satlas_mu_sensitivity(args: argparse.Namespace) -> None:
    ...
    c_mass = _col(raw, "mass") or _col(raw, "Mass") or _col(raw, "mass_value")
    if not c_teff or not c_logg:
        raise SystemExit("Input must contain Teff and logg columns for SATLAS mu-sensitivity test.")

    requested_targets = []
    if args.satlas_mu_targets:
        requested_targets = [t.strip() for t in str(args.satlas_mu_targets).split(",") if t.strip()]
        raw = raw[raw[c_target].astype(str).isin(requested_targets)].copy()
    if getattr(args, "limit", None) is not None:
        raw = raw.head(int(args.limit))

    mu_grid = [float(x.strip()) for x in str(args.satlas_mu_grid).split(",") if x.strip()]
    rows = []
    for _, row in raw.iterrows():
        target = str(row[c_target]).strip()
        teff = float(row[c_teff])
        logg = float(row[c_logg])
        feh = float(row[c_feh]) if c_feh and pd.notna(row[c_feh]) else np.nan
        mass = float(row[c_mass]) if c_mass and pd.notna(row[c_mass]) else np.nan
        for band in ("H", "K"):
            native_mu, native_I = _compute_satlas_cliv(teff, logg, band=band, mass=mass)
            for mu_min in mu_grid:
                mask = np.isfinite(native_mu) & np.isfinite(native_I) & (native_mu >= mu_min)
                mu_used = native_mu[mask]
                I_used = native_I[mask]
                bundle = band_coeffs_satlas(teff=teff, logg=logg, band_label=band, mass=mass, mu_min=mu_min)
                for law in ("linear", "power1", "power2", "quadratic"):
                    entry = bundle[law]
                    coeffs = np.asarray(entry.get("coeffs", []), dtype=float)
                    sigmas = np.asarray(entry.get("sigmas", []), dtype=float)
                    out = {
                        "target": target,
                        "band": band,
                        "teff": teff,
                        ...
                        "mass": mass,
                        "mu_min_test": mu_min,
                        "native_mu_min": float(np.nanmin(native_mu)) if native_mu.size else np.nan,
                        "used_mu_min": float(np.nanmin(mu_used)) if mu_used.size else np.nan,
                        "n_points_used": int(mu_used.size),
                        "law": law,
                        "note": entry.get("note", ""),
                    }
                    if law == "linear":
                        out["a"] = float(coeffs[0]) if coeffs.size >= 1 else np.nan
                        out["a_err"] = float(sigmas[0]) if sigmas.size >= 1 else np.nan
                    elif law == "power1":
                        out["alpha"] = float(coeffs[0]) if coeffs.size >= 1 else np.nan
                        out["alpha_err"] = float(sigmas[0]) if sigmas.size >= 1 else np.nan
                    elif law == "power2":
                        out["p1"] = float(coeffs[0]) if coeffs.size >= 1 else np.nan
                        out["p1_err"] = float(sigmas[0]) if sigmas.size >= 1 else np.nan
                        out["alpha1"] = float(coeffs[1]) if coeffs.size >= 2 else np.nan
                        out["alpha1_err"] = float(sigmas[1]) if sigmas.size >= 2 else np.nan
                    elif law == "quadratic":
                        out["u1"] = float(coeffs[0]) if coeffs.size >= 1 else np.nan
                        out["u1_err"] = float(sigmas[0]) if sigmas.size >= 1 else np.nan
                        out["u2"] = float(coeffs[1]) if coeffs.size >= 2 else np.nan
                        out["u2_err"] = float(sigmas[1]) if sigmas.size >= 2 else np.nan
                    rows.append(out)

    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    outfile = outdir / f"satlas_mu_sensitivity_{timestamp()}.csv"
    pd.DataFrame(rows).to_csv(outfile, index=False)
    print(f"[saved] {outfile} rows={len(rows)}")
...
def compute_one_model(
    ...
    *,
    ...
    if model_key in {'satlas', 'satlasross'}:
        use_rosseland_mu = model_key == 'satlasross'
        bundle['H'] = band_coeffs_satlas(
            teff=Teff, logg=logg, band_label='H', mass=mass, use_rosseland_mu=use_rosseland_mu
        )
        bundle['K'] = band_coeffs_satlas(
            teff=Teff, logg=logg, band_label='K', mass=mass, use_rosseland_mu=use_rosseland_mu
        )
        return bundle
    ...
    *,
    ...
    for _ in range(int(n_draws)):
        ...
        try:
            if model_key in {'satlas', 'satlasross'}:
                use_rosseland_mu = model_key == 'satlasross'
                h_bundle = band_coeffs_satlas(
                    teff=teff_i,
                    logg=logg_i,
                    band_label='H',
                    mass=mass,
                    use_rosseland_mu=use_rosseland_mu,
                )
                k_bundle = band_coeffs_satlas(
                    teff=teff_i,
                    logg=logg_i,
                    band_label='K',
                    mass=mass,
                ...
                h_lin = np.asarray(h_bundle['linear']['coeffs'], dtype=float)
                k_lin = np.asarray(k_bundle['linear']['coeffs'], dtype=float)
                h_quad = np.asarray(h_bundle['quadratic']['coeffs'], dtype=float)
                k_quad = np.asarray(k_bundle['quadratic']['coeffs'], dtype=float)
                h_vals = np.asarray(h_bundle['power1']['coeffs'], dtype=float)
                k_vals = np.asarray(k_bundle['power1']['coeffs'], dtype=float)
                h_pow2 = np.asarray(h_bundle['power2']['coeffs'], dtype=float)
                k_pow2 = np.asarray(k_bundle['power2']['coeffs'], dtype=float)
            else:
                sld = StellarLimbDarkening(
                    Teff=teff_i,
                    logg=logg_i,
...
def build_catalog(args: argparse.Namespace) -> None:
    ...
    for _, row in raw.iterrows():
        ...
        MH = float(row[c_feh])
        mass = float(row[c_mass]) if c_mass and pd.notna(row[c_mass]) else np.nan
        print(f"  Input params: Teff={Teff}, logg={logg}, [Fe/H]={MH}, mass={mass}")
    ...
    public_out = out_dir / OUTPUT_FILE
    ...
    out_wide = out_dir / f"{OUTPUT_FILE}_WIDE.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--satlas-mu-sensitivity", action="store_true", help="Run a SATLAS-only mu_min sensitivity test instead of the full catalog build.")
    parser.add_argument("--satlas-mu-grid", default="0.0,0.01,0.05,0.10,0.20", help="Comma-separated mu_min values for the SATLAS sensitivity test.")
    parser.add_argument("--satlas-mu-targets", default="", help="Optional comma-separated target list for the SATLAS sensitivity test.")
...
def main() -> None:
    args = parse_args()
    if args.satlas_mu_sensitivity:
        run_satlas_mu_sensitivity(args)
        return
    build_catalog(args)
