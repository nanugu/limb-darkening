...
DEFAULT_GRIDS = ("stagger", "kurucz", "mps1", "mps2", "satlasross")
...
def build_catalog(args: argparse.Namespace) -> Path:
    ...
    for idx, src_row in merged.iterrows():
        ...
        mass = as_float(src_row.get("mass", src_row.get("Mass")))

        missing_common = not (mircx_path and mystic_path and is_finite(diam_input) and is_finite(teff) and is_finite(logg))
        if missing_common:
            ...
            out_rows.append(row)
            print(f"[SKIP] {target_name}: missing files or Teff/logg/PL_diam")
            continue

        for grid in args.grids:
            grid_needs_feh = grid != "satlasross"
            if grid_needs_feh and not is_finite(feh):
                if "power1" in args.laws:
                    _blank_power1_columns(row, grid)
                if "power2" in args.laws:
                    _blank_power2_columns(row, grid)
                print(f"[SKIP] {target_name} {grid}: missing FeH")
                continue
            if "power1" in args.laws:
                try:
                    fit = fit_svam_target(
